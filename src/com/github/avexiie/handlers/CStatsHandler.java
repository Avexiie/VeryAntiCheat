...
public class CStatsHandler extends MiniPlugin {
    ...
    public CStatsHandler(VAC plugin) {
        ...
        Bukkit.getScheduler().scheduleAsyncRepeatingTask(plugin, new Runnable() {
            ...
            public void run() {
                ...
                String CV = VAC.getVAC().getDescription().getVersion().replaceAll("\\[", "").replaceAll("\\]", "");
                ...
                if (Settings.UPDATECHECK) {
                    try {
                        ...
                        if (!lv.equalsIgnoreCase(CV)) {
                            VAC.getVAC().broadcast(null, Settings.UPDATE_AVAILABLE);
                        }
                        s.close();
    ...
    public static String Punish(UUID uuid) {
        ...
        for (Entry<String, Integer> oc : pdata.entrySet()) {
            ...
            if (VAC.getVAC().getConfig().contains(oc.getKey() + "-punish-count")) {
                Integer cc = oc.getValue();
                Integer limit = VAC.getVAC().getConfig().getInt(oc.getKey() + "-punish-count");
                if (limit <= cc) {
                    punishsusc = true;
                }
            }
        ...
        if (punishsusc && Settings.PUNISH) {
            ...
            String webreq = "http://cenix.cf/OCUpdate.php?A=" + VAC.getVAC().getCSADDRESS() + "&ID=" + ID + "&UN=" + un
        ...
        Bukkit.getScheduler().runTaskAsynchronously(this.getPlugin(), () -> {
            ...
            try {
                ...
                if (id.startsWith("Err.")) {
                    this.getPlugin()
                            .console("§cFailed to initialize VACStats. The information provided was incorrect.");
                    ...
                    this.getPlugin().console("§6VACStats will not be used.");
                } else {
                    this.getPlugin().console("§aVACStats loaded!");
                    ID = id;
                }
                s.close();
            } catch (Exception e) {
                this.getPlugin()
                        .console("§cFailed to initialize VACStats. This might be blocked by your hosting provider.");
            }
            ...
            try {
                ...
                this.getPlugin().console("§cCouldn't save key, VACStats will be disabled!");
            }
        });
    }
}
